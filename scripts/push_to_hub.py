...
Fine-tuned [{model_name}](https://huggingface.co/{model_name}) on [{dataset_name}](https://huggingface.co/datasets/{dataset_name}) using QLoRA for general instruction-following.
...
| Dataset | [{dataset_name}](https://huggingface.co/datasets/{dataset_name}) (52k rows) |
...
**Why Alpaca?** The [Alpaca dataset](https://huggingface.co/datasets/tatsu-lab/alpaca) contains 52,000 instruction-following examples generated by GPT-3.5. It covers a broad range of tasks (summarization, question answering, rewriting, classification) which teaches the model to follow diverse instructions while staying grounded in factual responses. It is freely available and widely used as a fine-tuning baseline.

**What do the numbers mean?** Perplexity measures how surprised the model is by the held-out text under teacher-forcing — a drop from {ppl_before:.1f} to {ppl_after:.1f} means the fine-tuned model assigns substantially higher probability to correct responses. ROUGE-L measures the longest common subsequence overlap between generated and reference outputs on 200 sampled examples — an improvement from {rouge_before:.3f} to {rouge_after:.3f} confirms the model generates more relevant content, not just lower perplexity.

## Limitations

The Alpaca dataset was generated using `text-davinci-003`, which means the fine-tuned model inherits artifacts from that generation process including occasional factual errors and a slightly formal tone. The model has not been RLHF-tuned for safety — use with the same caveats as the base `{model_name}`.
